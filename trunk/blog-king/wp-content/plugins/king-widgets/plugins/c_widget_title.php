<?php
/**
	* @desc Output of  the the_title
	* @author Georg Leciejewski
	*/
	function c_widget_title($args, $number = 1) {
	...
	* @author Georg Leciejewski
	*/
	function c_widget_title_control($number) {
		// Get our options and see if we're handling a form submission.
		$options = $newoptions = get_option('c_widget_title');

		if ( $_POST["c_widget_title_submit_$number"] )
		{
			$newoptions[$number]['before_widget']	= html_entity_decode($_POST["c_widget_title_before_$number"]);
			$newoptions[$number]['after_widget']	= html_entity_decode($_POST["c_widget_title_after_$number"]);
		}
		if ( $options != $newoptions )
		{
			$options = $newoptions;
			update_option('c_widget_title', $options);
		}
		...
		echo king_get_textbox_p(array(
				'Label_Id_Name' 	=>"c_widget_title_before_$number",

				'Description' 	=> __('HTML before the Title', 'widgetContent'),
		...
		//after widget
		echo king_get_textbox_p(array(
				'Label_Id_Name' 	=>"c_widget_title_after_$number",
				
				'Description' 	=> __('HTML after the Title', 'widgetContent'),
		...
		echo king_get_hidden("c_widget_title_submit_$number",'1',"c_widget_title_submit_$number");
	}
	...
	* @author Georg Leciejewski
	*/
	function c_widget_title_setup() {
		$options = $newoptions = get_option('c_widget_title');

		if ( isset($_POST['c_widget_title_number_submit']) ) {
			$number = (int) $_POST['c_widget_title_number'];
		...
		if ( $options != $newoptions ) {
			$options = $newoptions;
			update_option('c_widget_title', $options);
			c_widget_title_register($options['number']);
		}
	}
	/**
	...
	*/
	function c_widget_title_page() {
		...
		?>
		<h2><?php _e('Title Widgets', 'widgetContent'); ?></h2>
		<?php
