...
function tguy_sm_add_admin_pages() {
	add_submenu_page('index.php', 'Such Statistik', 'Such Statistik', 1, __FILE__, 'tguy_sm_stats_page');
	add_options_page('Such Statistik', 'Such Statistik', 10, __FILE__, 'tguy_sm_options_page');
}
...
function tguy_sm_stats_table($results, $days, $do_include_successes = false) {
	...
	*/
	...
	if (count($results)) {
		...
		<tbody>
		<tr class="alternate"><th class="left">Wort</th><th>Suchen</th>
		<?php
		...
		?><p>Keine Statistiken f&uuml;r diese Periode</p><?php
	}
}
...
function tguy_sm_stats_page() {
	...
	<div class="wrap">
		...
		<h2>Alle Such-W&ouml;rter</h2>

		<p>Diese Tabelle zeigt die popul&auml;rsten Suchw&ouml;rter deiner Seite. <strong>Suchen</strong> zeigt an wie oft das Wort gesucht wurde und <strong>Hits</strong> zeigt wie viele Ergebnisse die <em>letzte</em> Suche geliefert hat</p>

		<div class="sm-stats-table">
		<h3>Gestern und Heute</h3>
		...
		<h2>Such-W&ouml;rter ohne Ergebnisse</h2>
		<p>Diese Tabelle zeigt nach welchen Worten erfolglos gesucht wurde. Vielleicht solltest du Artikel mit diesen Worten schreiben, um dem Besucher zu geben wonach er sucht.</p>
...
function tguy_sm_options_page() {
	...
	<?php
		...
		$tables = array();
			...
			if (!in_array($table_prefix.'searchmeter', $tables))
			{ ?> <h2>Install</h2>
				<form name="install" method="get">
				<input type="hidden" name="activate" value="true">
				<input type="submit" name="installieren" value="Aktivieren"></form>
	<?php	}
	?>
		<h2>Stastistiken L&ouml;schen</h2>

		<p>Du kannst mit diesem Button alle Suchstatistiken aus der Datenbank  l&ouml;schen.</p>

		<form name="tguy_sm_admin" method="post">
			<p>
			<input type="submit" name="tguy_sm_reset" value="L&ouml;sche Such-Statistiken" onclick="return confirm('Achtung alle Such-Statistiken werden gel&ouml;scht.\n  \'Cancel\' zum stop, \'OK\' zum l&ouml;schen');" />
			</p>
		</form>
...
if (!empty($_POST['tguy_sm_reset'])) {
	...
	header('Location: '.get_bloginfo('wpurl').'/wp-admin/options-general.php?page='.__FILE__.'&updated=true');
	exit();
}
